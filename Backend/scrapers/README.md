...
**Disclaimer: All of my python files were generated by Github Copilot.**

My scraper is named `Backend/scrapers/roux_scraper.py`, and produces a json file named `Backend/data/unprocessed/roux_institute_data.json`. The scraper collects data for people, institutions and departments as well as the WorksIn relation. One issue is that it does not output info for Project. The scraper is given the link to the Roux Institute "People" page, and finds all person cards on the page. It then follows a person's hyperlink if available and reads the person's information page. The scraper is supported by the `Backend/data/data_cleaning/extract_expertise.py` python script, which produces a secondary json file containing populated expertise fields. The expertise extraction attempts to use a LLM to extract skill keywords, but falls back on a keyword partial-matching option due to better performance of the latter method. The expertise extractor creates a new json file: `Backend/data/processed/roux_institute_data_with_expertise.json`.

## Running Roux scraper
To run roux scraper and save data in the correct folder, run
`python Backend/scrapers/roux_scraper.py --export-json Backend/data/processed/roux_people.json`.

To run the expertise extraction (after scraping), run 
`python Backend/data/data-cleaning/extract_expertise.py`.
